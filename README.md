...
- **Multi-source ISO scanning** - Dropbox, HTTP, S3, SFTP, and local folder sources
- **Direct-to-Proxmox download** - ISOs from HTTP/S3/Dropbox are pulled straight onto the Proxmox host (skipping the slow local-download-then-SCP path), with SCP fallback when the host has no internet
- **Integrity verification** - ISOs are de-duplicated and checksum-verified by MD5 on the Proxmox host before use
...
# Check Director status (add --insecure for self-signed Director appliances)
./versa-deployer status --director 192.168.1.50 --password yourpass
./versa-deployer status --director 192.168.1.50 --password yourpass --insecure
...
./versa-deployer add-source "https://bucket.s3.us-west-2.amazonaws.com/versa/"
...
| S3 | `https://bucket.s3.us-west-2.amazonaws.com/versa/` | Public S3 bucket (virtual-hosted, path-style, or `s3://` scheme) |
| SFTP | `sftp://user@host/path/to/isos/` | SSH-based file access |
| Local | `/path/to/local/isos` | Local filesystem path |

Sources are scanned recursively for `.iso` and `.md5` files. When an `.iso` has a matching `.md5`
companion, the tool verifies the checksum on the Proxmox host and reuses any identical image that
already exists (even under a different filename) instead of re-uploading.

## Configuration

Config, the auto-generated TLS cert/key, logs, and the downloaded-ISO cache are stored in a
per-user directory and persist across restarts:

| Platform | Location |
|----------|----------|
| Windows | `%AppData%\versa-proxmox-deployer\` |
| macOS | `~/Library/Application Support/versa-proxmox-deployer/` |
| Linux | `$XDG_CONFIG_HOME/versa-proxmox-deployer/` (or `~/.config/...`) |

A legacy `config.json` from the working directory (used by versions before 1.3.0) is migrated
automatically on first run. The `config.json` itself looks like:
...
- SSH access to Proxmox host (root or sudo user). VMs are created on the node you connect to;
  on a cluster, connect to the node you want to deploy on (cross-node placement is rejected
  rather than silently deploying to the wrong host)
